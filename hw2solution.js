...
function checkCar(car) {
    let resultString = `Your ${car.make} ${car.model} `;
    if (car.popular) {
        resultString += 'is popular. ';
    } else {
        resultString += 'is not popular. ';
    }
    if (car.mileage >= 100000) {
        resultString += 'It is not covered by warranty anymore!';
    } else {
        resultString += 'It is still covered by warranty!';
    }
    return resultString;
}
...
// И складывает их
function sumPositiveNumbers(num1, num2) {
    if (num1 < 0) {
        num1 = 0 - num1;
    }
    if (num2 < 0) {
...
function sumPositiveNumsOnly(num1, num2) {
    if (num1 < 0 || num2 < 0) {
